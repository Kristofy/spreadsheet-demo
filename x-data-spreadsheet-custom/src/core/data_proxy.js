...
export default class DataProxy {
  ...
  mergeXY(range) {
    const rn = range.eri-range.sri + 1;
    const cn = range.eci-range.sci + 1;
    // console.log('merge:', rn, cn);
    if (rn > 1 || cn > 1) {
      const { sri, sci } = range;
  ...
  unmergeXY(x, y) {
    const range = this.getCellRange(y, x);
    const cell = this.rows.getCell(range.sri, range.sci);
    if(cell && cell.merge){
      this.rows.deleteCell(range.sri, range.sci, 'merge');
      this.merges.deleteWithin(range);
    }else{
      console.error("can't unmerge");
    }
  }
